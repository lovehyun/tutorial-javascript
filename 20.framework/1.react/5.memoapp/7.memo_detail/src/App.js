...
const App = () => {
    ...
    const editMemo = (id, newText, newDetail) => {
        console.log("수정된 메모 ID:", id, "새 텍스트:", newText); // 디버깅 코드
        setMemos(
            memos.map((memo) =>
                memo.id === id ? { ...memo, text: newText, detail: newDetail } : memo
            )
        );
    };
    ...
    const saveMemoDetail = (id, newText, newDetail) => {
        editMemo(id, newText, newDetail);
        closeMemoDetail();
    };

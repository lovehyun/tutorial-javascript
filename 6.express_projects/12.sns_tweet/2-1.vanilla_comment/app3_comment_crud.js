...
// 댓글 작성
app.post('/api/comment', loginRequired, (req, res) => {
    const { tweet_id, content } = req.body;

    if (!tweet_id || !content) {
        return res.status(400).json({ error: '트윗 ID와 댓글 내용을 입력하세요.' });
    }

    const query = 'INSERT INTO comment (tweet_id, user_id, content) VALUES (?, ?, ?)';
    db.run(query, [tweet_id, req.session.user.id, content], function(err) {
        if (err) {
...
// 특정 트윗의 댓글 목록 조회
app.get('/api/comments/:tweet_id', (req, res) => {
    const tweetId = req.params.tweet_id;
    ...
    const query = `
        ...
        WHERE comment.tweet_id = ?
        ORDER BY comment.created_at ASC
    `;

    db.all(query, [tweetId], (err, comments) => {
        if (err) {
...
// 댓글 수정 API
app.patch('/api/comment/:comment_id', loginRequired, (req, res) => {
    ...
    db.get('SELECT * FROM comment WHERE id = ?', [commentId], (err, comment) => {
        if (err || !comment) {
        ...
        db.run('UPDATE comment SET content = ? WHERE id = ?', [content, commentId], function(err) {
            if (err) {
...
// 댓글 삭제 API
app.delete('/api/comment/:comment_id', loginRequired, (req, res) => {
    const commentId = req.params.comment_id;
    ...
    db.get('SELECT * FROM comment WHERE id = ?', [commentId], (err, comment) => {
        ...
        db.run('DELETE FROM comment WHERE id = ?', [commentId], function(err) {
            if (err) {

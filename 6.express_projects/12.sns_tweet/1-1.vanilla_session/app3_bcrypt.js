...
app.post('/api/profile/password', loginRequired, (req, res) => {
    ...
    db.get('SELECT * FROM user WHERE id = ?', [req.session.user.id], async (err, user) => {
        if (err || !user) {
        ...
        db.run('UPDATE user SET password = ? WHERE id = ?', [newHashedPassword, req.session.user.id], function(err) {
            if (err) {
